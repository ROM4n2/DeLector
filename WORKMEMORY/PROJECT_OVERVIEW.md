...
- 分支 `master`（与 origin 同步；v5.3.0 后已合入 PR #25 ADR-0005 备考域、#26 字体/弹窗修复、#27 遇见区 P0 + Go DAG job#1 + 评审修复；工作区干净）。
...
- 测试基线：**全量 686 passed + 1 skipped**（含 vocab_stats / encounter / 备份还原等新测试），10/10 `tools/*.mjs` 探针全绿；Go 门禁 `go test -race ./...` 与 `-tags integration` 全绿；pre-commit 密钥守卫有效，工作区干净。
...
- **开放待办**：① ~~v5.4.0 打包资产补录~~——已完成（2026-09-09 tag 构建自动挂载四平台资产：Windows ZIP / macOS·Linux TAR.GZ / Android arm64 APK，Release 已设 Latest；agent 预览包 artifact `DeLector-Agent-5.4.0-*`）；② 遇见区 A7 双端手工冒烟清单（桌面源码实例 + Android 各跑一遍）补齐 Master 最后一条 PENDING-作者 验收门（真 LLM 冒烟已完成 token 1,253）；③ 新功能候选：多模态听力微训 / 语料长难句强化立项；后续可评估喂文本/文本库跨端同步等开放项。
