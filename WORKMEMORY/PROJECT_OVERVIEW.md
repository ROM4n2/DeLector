...
- 发布面：正式版 **v5.6.0**；Android versionName 5.6.0 / versionCode 50600（CI 从 tag 推导）；桌面端正常，`python start.py` → `http://localhost:8000`。**v5.5.0 起 Android 端内置 4 篇 A1/A2 分级短文（首启自动 seed）；v5.5.1 修复 PWA 更新提示点不动/常驻遮挡（Android 需覆盖安装生效）；v5.5.2 收口 Ruff/Mypy 双门禁 + WiFi 手机拉取式内容分发（桌面→手机免发版更新卡包）；v5.6.0 听力微训工坊三模式上线（PR #44，`/api/listen` 四端点 + `listen_trials` 表，见上条），测试基线 **788 passed + 1 skipped**，**Android 真机 TTS 三模式点检已通过**（2026-09-14 作者 v5.6.0 覆盖安装验证，清单见 ops.md），README Roadmap 已更新。**
