...
- 发布面：正式版 **v5.5.2**；Android versionName 5.5.2 / versionCode 50502（CI 从 tag 推导）；桌面端正常，`python start.py` → `http://localhost:8000`。**v5.5.0 起 Android 端内置 4 篇 A1/A2 分级短文（首启自动 seed）；v5.5.1 修复 PWA 更新提示点不动/常驻遮挡（Android 需覆盖安装生效）；v5.5.2 收口 Ruff/Mypy 双门禁 + WiFi 手机拉取式内容分发（桌面→手机免发版更新卡包）。**
