...
- 分支 `master`（与 origin 同步；v5.3.0 后已合入 PR #25 ADR-0005 备考域、#26 字体/弹窗修复、#27 遇见区 P0 + Go DAG job#1 + 评审修复、#28 CI Hardening、#39 内容供给侧；v5.4.0/v5.5.0 均已发版并全资产挂载；工作区干净）。
...
- 发布面：正式版 **v5.5.0**；Android versionName 5.5.0 / versionCode 50500（CI 从 tag 推导）；桌面端正常，`python start.py` → `http://localhost:8000`。**v5.5.0 起 Android 端内置 4 篇 A1/A2 分级短文（首启自动 seed），覆盖安装即可读。**
