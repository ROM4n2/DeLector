...
- 分支 `master`（与 origin 同步；v5.3.0 后已合入 PR #25 ADR-0005 备考域、#26 字体/弹窗修复、#27 遇见区 P0 + Go DAG job#1 + 评审修复、#28 CI Hardening、#39 内容供给侧、#40 PWA 更新提示修复、#41 Ruff 清账、#42 WiFi 手机拉取式内容分发、#43 Mypy 类型门禁；v5.4.0/v5.5.0/v5.5.1/v5.5.2 均已发版并全资产挂载；工作区干净）。
...
- 测试基线：**全量 839 passed + 1 skipped**（Mypy 类型门禁零错误，起点 138）（含听力微训 35 支：引擎 21 + API 9 + 探针 2 + 模块图 3；长难句精读 51 支：引擎 22 + API 16 + 探针 5 + 模块图 8——v5.7.2 跨端打包守卫 +1、v5.7.4 pure 启发式 +5 + spacy-status +1、v5.7.6 诊断 getter 回归 +1；WiFi 内容分发 32 支：拉取端点 15 + 前端探针 17；Ruff 守卫 5 支；A7 服务端 E2E 6 支、客户端行为探针 5 支、seed 契约 11 支），10/10 `tools/*.mjs` 探针全绿；Go 门禁 `go vet` + `gofmt -l`（空）+ `go test -race ./...`（8 包）全绿；**Python 门禁 `ruff check .` 零告警**（2026-09-13 起，起点 2265）；pre-commit 密钥守卫有效，工作区干净。
