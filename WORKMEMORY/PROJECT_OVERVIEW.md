...
- 分支 `master`（**tag `v5.3.0` 已 push**；其后 Phase 1 重构 + vault-grill 修正 + Phase 2a Go Agent Runtime 已全部推送，工作区干净）。
...
- **Sub-Plan B · Go DAG job#1 真执行（2026-09-07 收官，分支 `feature/encounter-job1`）**：agent 从「只装配不执行」变**首个真执行 job#1 落地**——`delector job run encounter-pack` 扫描本地语料 → 去重/限量/并发逐篇 DAG（analyze→vocab_stats→gloss(DeepSeek)→export）→ 可选投递 `POST /api/encounter/import-pack`。落地：`internal/corpus`（B2）、`internal/job`（B3 pack schema+预算 / B4 gloss / B5 encounter DAG / B6 runner 并发预算退避）/ B7 cobra 接线（registry **6 工具**含 `vocab_stats`、Python 侧同）+ B8 `-tags integration` 真链路门禁（`TestEncounterRealChain` PASS：真 python 工具 + stub LLM + import-pack + texts 命中）。测试：agent/ `-race` 全绿、`-tags integration` 全绿；Python 全量 **686 passed + 1 skipped** 零回退。冒烟：CLI dry-run 与真实 run（stub LLM）均 `packs=2 failed=0` 退出 0；真实 DeepSeek 档 PENDING-作者（无 key）。完整状态与偏差（5 条）见 `docs/plans/2026-09-07-dag-job1-content-producer.md`「执行状态（Sub-Plan B 收官）」。
- **开放待办**：① v5.3.0 打包资产——Windows/macOS/Linux 便携包与 Android APK 待打包、GitHub Release 资产待补录（README 下载表已标注「源码版·打包中」）；② 新功能候选：多模态听力微训 / 语料长难句强化立项；③ 遇见区 job#1（Sub-Plan B）真 LLM 冒烟 + 双端手工——Go DAG job#1 真链路代码与门禁已收官（见上），作者桌面配 `DEEPSEEK_API_KEY` 后真跑 gloss 并记录 token 消耗、再跑 A7 双端手工清单补齐 Master 剩余两条 PENDING-作者 验收门；后续可评估喂文本/文本库跨端同步等开放项。
