...
- 发布面：正式版 **v5.7.4**；Android versionName 5.7.4 / versionCode 50704（CI 从 tag 推导）；桌面端正常，`python start.py` → `http://localhost:8000`。**v5.5.0 起 Android 端内置 4 篇 A1/A2 分级短文（首启自动 seed）；v5.5.1 修复 PWA 更新提示点不动/常驻遮挡（Android 需覆盖安装生效）；v5.5.2 收口 Ruff/Mypy 双门禁 + WiFi 手机拉取式内容分发（桌面→手机免发版更新卡包）；v5.6.0 听力微训工坊三模式上线（PR #44，`/api/listen` 四端点 + `listen_trials` 表，见上条），**Android 真机 TTS 三模式点检已通过**（2026-09-14 作者 v5.6.0 覆盖安装验证，清单见 ops.md）；v5.7.0 长难句精读工坊上线（PR #45，`/api/syntax` 三端点 + `hard_sentence_trials` 表 + `syntax_score` 引擎，见上条），测试基线 **828 passed + 1 skipped**；v5.7.1 热修长难句工坊真机 500（detail 降级 404 + all 逐材料隔离 + 切句容错），测试基线 **831 passed + 1 skipped**；v5.7.2 真根因修复（rank_sentences 弃用 pydantic v2 专属 `model_copy`，Android 打包 pydantic 1.x 兼容 + 跨端打包兼容守卫），测试基线 **832 passed + 1 skipped**；v5.7.3 揭示句法树 `[object Object]` 渲染修复（field_texts 字符串兼容 + 探针 fixture 真实形状 + 难度分文案消歧），测试基线 **832 passed + 1 skipped**；v5.7.4 pure 降级评分增强 + 前端字号优化 + 轻量分析文案 + spaCy 加载诊断端点（`/api/syntax/spacy-status`），测试基线 **838 passed + 1 skipped**，README Roadmap 已更新。**
