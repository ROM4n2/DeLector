...
ROOT = (
    Path(__file__).resolve().parent.parent if Path(__file__).parent.name == "tools" else Path(__file__).resolve().parent
)
...
def check_security():
    ...
    if hook_path.exists():
        ...
        record_issue(
            "SEC", "缺少 .githooks/pre-commit 密钥拦截钩子", "从模板创建并配置 git config core.hooksPath .githooks"
        )
    ...
    try:
        ...
        for tbl in core_tables:
            if tbl not in _BACKUP_TABLES:
                record_issue(
                    "DATA",
                    f"数据库核心表 [{tbl}] 未在 _BACKUP_TABLES 中注册！",
                ...
                if missing_in_backup:
                    record_issue(
                        "DATA",
                        f"表 [{tbl}] 在 _BACKUP_TABLES 规范中遗漏字段: {missing_in_backup}",
        ...
        if "plural" in req_fields:
            ...
            record_issue(
                "DATA",
                "VocabCardReq 遗漏 plural 字段，导致生词卡复数无法入库",
...
def check_db_concurrency():
    ...
    try:
        ...
        if jmode.lower() in ("wal", "memory"):
            ...
            record_issue(
                "DB",
                f"主库使用 {jmode} 模式而非 WAL 模式，高并发读写可能锁库",
...
def check_nlp_and_linguistics():
    ...
    try:
        ...
        record_pass(
            "NLP", f"文本分析流水线就绪，提取 {doc['stats']['word_count']} 词，评级 {doc['stats']['recommended_level']}"
        )
...
def check_hygiene_and_tests():
    ...
    try:
        res = subprocess.run(
            [
                ...
                "pyflakes",
                "delector/server.py",
                "delector/core/database.py",
                "delector/nlp.py",
                "delector/linguistics.py",
                "delector/syntax_tree.py",
    ...
    try:
        res = subprocess.run(
            [sys.executable, "-m", "pytest", "--collect-only", "-q"], capture_output=True, text=True, cwd=str(ROOT)
        )
