...
| 项 | 值 |
|---|---|
| 当前分支 / HEAD | `fix/ssrf-pinned-ipv6-ranges`（v4.4.8 的 tag 已推、**但 CI Linux job 红、Release 未发布**，本分支修的就是那条挂掉的测试） |
...
| 对外发布 | 已发布至 **v4.4.7**（四平台产物齐、CI 验签闸过）；**v4.4.8 tag 已推但未发布** —— run `32866451079` 的 Linux x64 job 红，`Publish GitHub Release` 被跳过，macOS / Windows / Android 三个 job 都过了。挂的是 SSRF 那条新测试自己：它在本机（3.11.8）靠 `ipaddress` 一条粗粒度的 `2001::/23` 私有段条目**巧合**为绿，CI（3.11.16）把该条目换成细粒度条目后 `2001:20::/28` 掉了出来 —— **同一份代码、同一个 3.11 大版本，判定相反**。改法是把段钉在自己代码里，不是删断言。⚠️ Java 仍未经本机编译（无 Android SDK），运行时行为只能真机验收；**v4.4.6 / v4.4.7 / v4.4.8 的真机验收合并做一次**（装 v4.4.6 → 覆盖装 v4.4.8，中间不卸载），要点见「已知问题 / 待办」 |
| 未完成的事 | 见文末「已知问题 / 待办」 |
