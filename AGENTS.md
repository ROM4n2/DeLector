...
| 项 | 值 |
|---|---|
| 当前分支 / HEAD | `master`（**v4.4.8**：离线词库补齐 + SSRF 闸判定修正；PR #8 未定义 CSS 变量、PR #9 词头归一化与 SSRF 闸已合入） |
| 测试 | **264 / 264 全绿**（2026-08-25 合入后在 `master` 上实测，`pyflakes` 干净）。新增：`test_dict_pipeline.py` 10 条锁词头归一化契约、12 条 IPv6 过渡格式 SSRF 断言；**两条依赖真实 DNS 的测试已钉住解析结果**，不再随本机 IPv6 状态时红时绿 |
...
| 对外发布 | 已发布至 **v4.4.7**（四平台产物齐、CI 验签闸过）；**v4.4.8 待发布**（版本号 40408，仅 arm64-v8a）。⚠️ Java 仍未经本机编译（无 Android SDK），运行时行为只能真机验收；**v4.4.6 / v4.4.7 / v4.4.8 的真机验收合并做一次**（装 v4.4.6 → 覆盖装 v4.4.8，中间不卸载），要点见「已知问题 / 待办」 |
| 未完成的事 | 见文末「已知问题 / 待办」 |
