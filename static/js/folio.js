...
const GERMAN_MOTTOS = [
  ...
  {
    de: "Ein Buch ist wie ein Garten, den man in der Tasche trägt.",
    zh: "一本书如同一座随身携带的私家花园。",
    author: "Arabisches Sprichwort auf Deutsch",
  },
];
  ...
  if (bottomTrack) {
    ...
    const statItems = [
      ...
      { icon: "⏱", label: "ZEIT", val: `${minutes} MIN`, desc: "专注学时" },
      {
        icon: "🗂",
        label: "KARTEN",
        val: `${totalCards} GESAMT`,
        desc: "卡片总库",
      },
    ];

    const statHtml = statItems
      .map(
        (s) => `
...
export async function loadProgress() {
  const dayIndex =
    Math.floor(Date.now() / (1000 * 60 * 60 * 24)) % GERMAN_MOTTOS.length;
  ...
  try {
    ...
    const streak = stats.streak || 0;
    const lastStreak = parseInt(
      localStorage.getItem("delector_streak_celebrated") || "0",
      10,
    );
    ...
    if (heatbar) {
      if (totalCards === 0) {
        heatbar.innerHTML =
          '<div class="heat-seg" style="flex:1;background:var(--paper-deep);text-align:center;color:var(--pencil);font-size:0.6875rem;padding:0.35rem;">暂无卡片数据 (Keine Kartendaten)</div>';
      } else {
    ...
    if (cefrLadder) {
      const levels = [
        {
          code: "A1",
          name: "入门级 (Elementar A1)",
          color: "var(--hl-A1)",
          ink: "var(--hl-A1-ink)",
        },
        {
          code: "A2",
          name: "基础级 (Grundstufe A2)",
          color: "var(--hl-A2)",
          ink: "var(--hl-A2-ink)",
        },
        {
          code: "B1",
          name: "进阶级 (Mittelstufe B1)",
          color: "var(--hl-B1)",
          ink: "var(--hl-B1-ink)",
        },
        {
          code: "B2",
          name: "高阶级 (Oberstufe B2)",
          color: "var(--hl-B2)",
          ink: "var(--hl-B2-ink)",
        },
        {
          code: "C1",
          name: "精通级 (Fortgeschritten C1)",
          color: "var(--hl-C1)",
          ink: "var(--hl-C1-ink)",
        },
      ];

      cefrLadder.innerHTML = levels
        .map((l) => {
    ...
    if (errorsWrap && errorsList) {
      if (topErrors.length > 0) {
        errorsList.innerHTML = topErrors
          .map(
            (e) => `
...
export function renderTrendChart(trend) {
  ...
  const maxVal = Math.max(
    ...trend.map((d) => (d.cards_added || 0) + (d.cards_mastered || 0)),
    4,
  );
