...
def export_anki_deck(output_path: str, db_path: str = DB_PATH) -> str:
    ...
    for r in vocab_rows:
        ...
        deck.add_note(
            genanki.Note(model=VOCAB_MODEL, fields=[styled_front, r["word"], r["lemma"], meta, r["definition_zh"]])
        )
...
def list_articles():
    with get_db() as conn:
        rows = conn.execute(
            "SELECT id, title, created_at, length(raw_text) as char_count FROM articles ORDER BY id DESC"
        ).fetchall()
