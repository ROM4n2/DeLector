...
## 实现注记（2026-09-14 T6 回填）

- **`path` 标注来源（实现层补全）**：`syntax_tree.analyze_syntax_tree` 双路径函数的返回 dict 实测**不带 `path` 键**；`rank_sentences` 按实际调用路径增量探测并注入 `path`（spacy/pure）——响应带 `path`、降级仅参考的语义与 §3 红线 1 一致，无规格冲突。
- **入复习盒**：前端直拼 `POST /api/cards/grammar`（`saveGrammar` 语义 → `grammar_cards`），不新增端点——与 §2/§3「复用既有端点 / 不新建卡种表」一致，无规格偏差。
